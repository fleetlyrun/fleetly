...
3. 契约：OpenAPI 生成客户端编译 + oasdiff breaking 检查；MCP 工具 schema 快照；错误码注册表校验（只增、不复用）；edgesets.yaml JSON Schema 样本回归
4. 许可证守卫：默认发行组件清单不得出现 AGPL/DSAL（白名单机制，见 D5）
5. 集成 E2E（单节点 dind）：`docker:29.8.1-dind` 内 install → 部署 fixture 应用 → health gate → 路由 → 回滚 → 平台自升级
...
4. 兼容承诺检查（edgesets.yaml v1 / REST `/v1` / N-2 升级路径）+ changelog
5. 文档更新检查（含升级说明）
