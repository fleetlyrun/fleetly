...
| 状态 | 日期 | 说明 |
|---|---|---|
| **已实现+真机验证（2026-09-22，W5 收官）**：S1 日志库/S2 检索+访问日志/S3 metrics opt-in/S4 通知全落；staging 真机演练全过（[runbook §11](../runbooks/vps-dogfooding.md)）——VL 升级即部署 ingest ok/检索命中/访问日志归因 deployment_id/HMAC 5/5 重算/零回环；**预算实测：默认面（VL on）≈492MB<600MB ✓（VL 仅 12.7MB）、metrics on 态 ≈671MB 超顶（opt-in 裁决被实测验证，VM 收紧挂账）**。真机修复二件：cadvisor containerd socket 双宿主形态自适选择（dind=dockerd 私有 / 宿主=系统 containerd——`/bin/sh -c` 启动期择一，修复后每容器序列恢复；swarm 归属标签与 image 归因标签的边界实测入 §6 挂账） | 2026-09-22 | 裁决输入：[v0.2 规划 §2 W5 行](../plan/2026-09-20-v0.2-plan.md)、V2-1（VictoriaLogs 默认捆绑，用户直裁 2026-09-20）、V2-6（通知 Webhook 首发）、R4 载体修订（访问日志随 VL 入日志库，规划 §7 修订 4）；本票新增裁决：**D-W5-2**（metrics opt-in 管理组件，用户直裁 2026-09-22——依据：staging 429MB 基线 + VL 后 ≈455MB，三件套 +110~165MB 默认捆绑 ≈589MB 踩线无余量）、**D-W5-4**（受管服务宿主可达性 = host-mode 回环发布）。同轮关联裁决：**D-W5-1**（E2 MCP 暂缓出 v0.2，用户直裁「CLI 目前基本够用」→ E2 整票移 v0.3，规划 §5/§6 已回填）。W5 其余两票见 [web-terminal 专项](2026-09-22-web-terminal.md)（D-W5-3 通道修订 + 控制面 TLS）。**S1 落地注记（三处实现偏离，均真机取证）**：①D-W5-4 字面「PortConfig.HostIP=127.0.0.1」不可实现——Engine API 对 swarm 服务端口 HostIp 静默丢弃（docker 29.8.1 dind 实证，moby v1.56 无该字段），落地 = **host 网络任务 + VL 原生 `-httpListenAddr=127.0.0.1:9428`**（进程级回环绑定，零公网面不变量比设计字面更严；附带：host 网络任务不可挂 overlay，`fleetly-victorialogs-net` 留位网络不创建——未来容器消费方的访问面另裁）；②镜像 repo 更正 `victoriametrics/victoria-logs`（设计字面 `victorialogs/victoria-logs` 在 Docker Hub 不存在），钉 **v1.52.0** index digest；③ES bulk 端点实测 `/insert/elasticsearch/_bulk`（`/insert/elasticbulk` 在 v1.52 无此端点）。**S2 落地注记**：RouterName 反解用「本轮 active apps × 服务按 ingress 公式构造候选集」的确定性候选匹配（app/service 含 '-' 时字符串切分有歧义）；e2e 夹具三教训入脚本注记——build context 须在受管根（H14：/tmp/git root）、deploy 不自动构建（先 `fleetly build` 再 deploy 的直通核对语义）、alpine busybox 无 httpd applet（HTTP 面载体用 whoami 钉版镜像）。**S3 落地注记（§4 对应）**：①**拓扑偏离（D-W5-4 同族）**——设计字面「VM host-mode 回环发布 + 采集器挂 fleetly-metrics-net、VM 抓 tasks.*」不可实现：host 网络任务不能挂 overlay、宿主命名空间解析不了 tasks.*（Docker 内嵌 DNS 不服务宿主）；落地 = **三件全部 host 网络任务 + 进程原生回环监听**（VM `-httpListenAddr=127.0.0.1:8428`、cAdvisor `-listen_ip=127.0.0.1 -port=8080`、node_exporter `--web.listen-address=127.0.0.1:9100`，flag 名均镜像 -help 实测），抓取 targets=127.0.0.1——**跨节点采集结构性缺席（v0.2 = manager 节点指标；worker 修订票见 §6）**，`N/M nodes reporting` 与 Console 常驻诚实文案；②VM 单机版抓取 flag 实为 `-promscrape.config=<文件路径>`（`-prometheus.config` 不存在）→ 落地 **swarm config 对象内容寻址分发**（`fleetly-vm-scrape-<sha8>`，引用需 ID+名双写——W3 secret-ID 同族教训再证）；③cAdvisor repo 勘误：docker.io `google/cadvisor` 已废弃（末版停 2019）→ 钉 **gcr.io/cadvisor/cadvisor:v0.55.1**；docker29 三坑均实证化解（镜像自带 healthcheck 双栈走 ::1 被 IPv4 探针覆盖、containerd 托管 socket 需显式指参、**snapshotter 形态 swarm 归属标签缺席 = 应用归因降级**——Console 诚实空态+降级说明，恢复随 cAdvisor 上游，e2e A13 探针持续记录）；④S1 VL duty「网络 target 被 engine 归一为 ID → 每拍误判漂移 + deployed 事件重发」缺陷在 S3 门上修复（NetworkName 反解回名同锚比对，稳态零写测试钉住）。**S4 落地注记（§5 对应）**：①退避表读法钉定 = **总尝试 3 次**（首发 + 30s + 5m 退避两段重试后终态 failed，「当日了结」不变；30m 档为 4 次读法下的设计冗余，不启用）；②投递器**首启游标对齐当前最大 seq**（订阅从现在开始、不回放历史——升级安装不轰炸接收方）；③消费间隙（游标过期）= warn + 对齐最大 seq，零合成台账行（间隙是全局保留期事实，非端点投递失败）；④secret 指纹落库为 `webhook_endpoints.secret_fingerprint` 列（app_secrets.hash8 同型，读面免解密）；⑤scope：读=read、写（含 Rotate/Test）=admin；⑥e2e receiver 形态 = dind 内 busybox nc 循环落盘请求原文 + 宿主 python 独立重算 HMAC 验签（非 grep 签名存在）；⑦停用端点的到期 pending 行 30s 推迟不烧尝试预算，7d janitor 兜底 |
