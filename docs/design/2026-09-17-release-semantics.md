...
| D-REL-9 | 治理参数取当前平台配置、secret 值取当前、非密钥 env 随快照回滚（三层合并结果，见架构 §2.4） | 前瞻设置不随版本回滚；密钥回滚是安全事故；非密钥 env 属于 workload，不回滚会让「完整快照」名不副实（env 变更必须经部署固化，天然形成可回滚点） | 快照含治理参数/密钥（静默回滚与安全事故）；env 取当前（回滚半到位） | 独立收敛 |
