...
- 控制面与 swarm raft 是两个备份源，备份时间点必然不同；恢复顺序与「恢复期是否自动收敛」未定义（自动收敛在 DB 较旧时会静默回滚部署）。
...
| D-STM-6 | 孤儿只登记不删除；恢复后只读观察 | DB 滞后于底座是 DR 的必然状态；恢复期自动收敛会静默回滚部署 | 按 DB 反向清理；恢复即收敛 | 独立收敛 |
...
| 恢复后孤儿容器命运（raft 回退到旧点） | 残留任务/端口冲突 | **V5b 已实测（2026-09-17 Spike C C6）**：raft 回退后——备份之后新建的容器在 worker 重连时立即被 GC（t0 消失不复现）；备份之前已删除的服务会**复活**（原卷原数据可读）；孤儿卷残留不自动清理——「恢复后只读观察 + 人工处理差异」的必要性得到实证，热备频率结论维持 |
