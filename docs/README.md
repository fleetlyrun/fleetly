...
| 2026-09-23 | [团队与多用户 RBAC（用户/团队/项目）设计](design/2026-09-23-rbac-teams.md) | 已裁决（方案冻结，W0 完成） | 用户注册登录（首用户=平台管理员+**存量自动全量认领** D-W0-5 用户直裁）/Team+Project 多租户（注册默认个人队/邀请链接/四档角色 viewer-developer-admin-owner+平台管理员分离；**Project 层角色 = 队内覆写形**（D-W0-2 修订：仅限团队成员双向覆写、owner 不可覆写、可见性零级联；A 形全量挂账））/项目隔离（**v0.2 每 app 专属 overlay 底座已结构性具备**，准入守门收敛到 E4 库网络一处）/双门准入（token scope+角色，fail-closed）/审计 90d 可调+双读面/FZ-12 指纹披露+变更事件；D-W0-1~9 全落（含同名项目跨团队 = per-team 唯一 + 限定形引用解析；D-W0-2/D-W0-4 修订：项目角色队内覆写形、app/库名 team 内唯一+底座命名 team 段化〔卷公式不变零数据迁移+升级收编〕；商业线 = **开源全功能，付费=云+企业件** D-W0-7） |
