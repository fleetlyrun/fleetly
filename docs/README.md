...
| 2026-09-20 | [v0.2 实施规划](plan/2026-09-20-v0.2-plan.md) | 规划草案（待剩余裁决轮） | 波次 W0-W5（裁决轮/引擎加固 → dogfooding → 多节点 → S3+Cron → 数据库托管 → MCP/观测/终端）；已裁决 V2-1（易用性提级、预算顶 600MB、VictoriaLogs 默认捆绑）+ V2-2（RustFS 1.0 opt-in 引入、同节点备份诚实标注、跨节点互备挂账 E1 后）；剩余 D-V2-1~7；≈64-104 人日 |
