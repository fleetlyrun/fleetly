...
func TestEnvReservedPrefixGuard(t *testing.T) {
	...
	_, err := st.SetAppEnv(ctx, appID, "FLEETLY_DB_X", "cipher-secret-value", "platform", "system")
	if err == nil {
	...
	// 2. 用户写普通键照常。
	if _, err := st.SetAppEnv(ctx, appID, "DATABASE_URL", "cipher-ok", "platform", "system"); err != nil {
		t.Fatalf("platform write of normal key: %v", err)
	}

	// 3. system 物化写保留前缀放行（S4 连接串物化通道）。
	if _, err := st.SetAppEnv(ctx, appID, "FLEETLY_DB_PG_PROD_URL", "cipher-conn", "system", "system"); err != nil {
	...
	if _, err := st.SetAppEnv(ctx, appID, "FLEETLY_DB_PG_PROD_URL", "cipher-hijack", "platform", "system"); err == nil {
	...
	if _, err := st.SetAppEnv(ctx, appID, "fleetly_db_x", "cipher-lower", "platform", "system"); err != nil {
		t.Fatalf("platform write of lowercase fleetly_db_x: %v", err)
	}

	// 6. system 写任意键放行（平台内部物化不限 FLEETLY_ 前缀）。
	if _, err := st.SetAppEnv(ctx, appID, "PLAIN_SYSTEM_KEY", "cipher-sys", "system", "system"); err != nil {
