...
-- phase_started_at——queued → preparing 转换（引擎拾取）时刻的预算基线。
--
...
--
-- 语义：基线随拾取原子写入（CAS 补丁同拍），预算自拾取时刻起算，排队
-- 等待不计入；存量行（NULL）回落 created_at 保持旧语义。releasing 自有
-- release_started_at / watchdog_deadline_at 时间基线，与本列互不干扰——
-- blocked_waiting 等待期间基线不重置也无消费（等待不计入预算即本意）。
