...
// jobState 构造一个 job 服务的实况投影。
func jobState(name, app, process string) engine.ServiceState {
	return engine.ServiceState{
		Name:   name,
		Labels: map[string]string{state.LabelApp: app, state.LabelProcess: process},
	}
}

// testJobName 构造确定性 job 服务名（固定 ULID 词形尾缀）。
func testJobName(t *testing.T) string {
	t.Helper()
	name, err := naming.CronJobName("cronapp", "task", "01JABCDEFGHJKMNPQRSTVWX")
	if err != nil {
...
func TestCronJobRefOfMapping(t *testing.T) {
	jobName := testJobName(t)
	ref, ok := cronJobRefOf("cronapp", jobState(jobName, "cronapp", "task"))
	if !ok {
	...
	if _, ok := cronJobRefOf("cronapp", jobState("fleetly-cronapp-task", "cronapp", "task")); ok {
		t.Fatal("long-running service name accepted as job")
	}
	if _, ok := cronJobRefOf("otherapp", jobState(jobName, "cronapp", "task")); ok {
		t.Fatal("app label drift accepted (mis-attribution)")
	}
	if _, ok := cronJobRefOf("cronapp", jobState(jobName, "cronapp", "")); ok {
		t.Fatal("missing process label accepted")
	}
	if _, ok := cronJobRefOf("cronapp", engine.ServiceState{Name: jobName}); ok {
...
func TestCronJobLogsCollectedFromStart(t *testing.T) {
	...
	port.setCronJobs(app.Name, jobState(jobName, app.Name, "task"))

	mg.scanOnce(ctx)

	// ring 面（Follow 同源）：行归属 (cronapp, task)。cancel 先于排空——
	// 订阅 channel 由 cancel 关闭（回放已在容量充足的缓冲内，锁内投递不丢）。
	ch, cancel := mg.hub.subscribe(app.Name, "task")
	cancel()
	var snap []Entry
	for e := range ch {
		snap = append(snap, e)
	...
	if snap[0].App != app.Name || snap[0].Service != "task" || snap[0].Line != "cron-e2e-log-marker" {
	...
	mg.mu.Lock()
	_, hasJobCursor := mg.streams[cronCursorKey(app.Name, jobName)]
...
func TestCronJobLogsNoDupAndCursorReclaimed(t *testing.T) {
	...
	port.setCronJobs(app.Name)
	mg.scanOnce(ctx)
	mg.mu.Lock()
	_, stillThere := mg.streams[cronCursorKey(app.Name, jobName)]
	mg.mu.Unlock()
	if stillThere {
...
func TestCronJobDiscoveryTransientKeepsCursor(t *testing.T) {
	...
	mg.mu.Lock()
	_, kept := mg.streams[cronCursorKey(app.Name, jobName)]
	mg.mu.Unlock()
	if !kept {
