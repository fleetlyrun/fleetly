...
// 对 80/443 探测（本机视角）→ 如实报告当前解析 IP 与端口可达性。不算
// DNS 传播（无法也不应猜测全网视角）——契约（A 记录指向全部节点、
// TTL ≤300s）的核验材料 = 当前观测，判断权在操作者。
...
// DomainCheck 是单域名的验证报告。
type DomainCheck struct {
	Domain string   `json:"domain"`
	IPs    []string `json:"ips"`
	// Resolved 报告本机是否解析得到至少一个地址。
	Resolved bool `json:"resolved"`
	// HTTP80 是 80 端口探测结果（"" = 不可达；否则记录响应状态行）。
	HTTP80 string `json:"http_80,omitempty"`
	// HTTPS443 是 443 端口 TLS 握手结果（"" = 不可达）。
...
// 5 × 每应用 10 的契约上界，串行足够）。
func VerifyDomains(ctx context.Context, domains []string) []DomainCheck {
	out := make([]DomainCheck, 0, len(domains))
	for _, d := range domains {
		out = append(out, verifyOne(ctx, d))
	}
	return out
}

// verifyOne 验证单域名。
func verifyOne(ctx context.Context, domain string) DomainCheck {
	...
	// 80：带 Host 头的 GET（经 Traefik 的路由语义即请求头驱动）。
	client := &http.Client{
		Timeout: verifyTimeout,
		Transport: &http.Transport{
			DialContext: func(dctx context.Context, _, _ string) (net.Conn, error) {
				var d net.Dialer
				return d.DialContext(dctx, "tcp", net.JoinHostPort(ip, "80"))
			},
		},
	}
	...
	// 443：TLS 握手（SNI = 域名；证书链照实记录，不做信任判定）。
	...
	conn, err := dialer.DialContext(vctx, "tcp", net.JoinHostPort(ip, "443"))
	if err != nil {
