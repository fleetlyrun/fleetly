...
// 打印到 pusher stderr，推送照常成功。FLEETLY_PUSH_USER 由 daemon 在
// git-receive-pack 子进程环境注入（SSH 公钥认证回调解析的属主，W2 §2.3
// push 审计 actor 署名；缺省 = 存量无主键/克隆路径——payload 落空串，
// daemon 按原口径处理）。
...
post() {
  payload=$(printf '{"sha":"%s","ref":"%s","push_user":"%s"}' "$1" "$2" "${FLEETLY_PUSH_USER-}")
