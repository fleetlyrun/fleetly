...
func TestNewServicesStopOrder(t *testing.T) {
	...
	services := NewServices(app, st, id, ob, jr, bm, box, q, builder, eng,
		ing, lm, cm, src, cfg, nil, nil, nil, hs, gs)
	...
	want := []string{
		...
		// 在途快照，rustfs/victorialogs/database 收敛 duty、cron 调度器
		// 同层，store 殿后）。
		"state.identity", "state.observer", "state.janitor",
		"state.backup", "objectstore.rustfs", "logs.victorialogs", "database.converge", "cron.schedule", "state.secrets", "state.store",
	}
