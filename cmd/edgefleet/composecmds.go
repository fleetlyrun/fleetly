...
func (c *validateCmd) Synopsis() string {
	return "validate a compose file against the edgefleet controlled subset"
}
...
// planCmd 实现 edgefleet plan <path>：校验 + 归一化 + 与基线 diff，输出
...
// diffCmd 实现 edgefleet diff <a> <b>：两份 compose 归一化差异。退出码与
// plan 同三态。
type diffCmd struct {
	jsonOut bool
}
