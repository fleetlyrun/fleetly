...
// 状态库直读直写（与 edgefleetd 同一 state 层语义）；nodes 命令是观测
...
import (
	...
	"github.com/edgesets/edgefleet/internal/placement"
	"github.com/edgesets/edgefleet/internal/secrets"
	"github.com/edgesets/edgefleet/internal/state"
)

// dbDefaults 是状态库直连命令的公共缺省（与 edgefleetd config 缺省一致）。
const (
	defaultDBPath  = "./edgefleet.db"
	defaultKeyPath = "./edgefleet.key"
)
...
// loadBox 显式加载主密钥；allowCreate 允许首次生成（与 edgefleetd 首启
...
func loadBox(path string, allowCreate bool) (*secrets.Box, bool, error) {
	if _, err := os.Stat(path); errors.Is(err, os.ErrNotExist) && !allowCreate {
		return nil, false, fmt.Errorf("master key %s 不存在：先启动 edgefleetd（首启生成），写路径可显式传 --create-key", path)
	}
...
// envSetCmd 实现 `edgefleet env set <app> <key> <value>`：明文经 age envelope
...
// envGetCmd 实现 `edgefleet env get <app> <key>`：显式读回明文（操作者主动
// 取值路径；列表输出不落值）。
...
// envListCmd 实现 `edgefleet env list <app>`：键名 + 来源 + 状态位；值恒
// 脱敏（读值走 env get 显式路径）。
...
// envRmCmd 实现 `edgefleet env rm <app> <key>`（生效语义：当前运行 env 不变，
...
// placementShowCmd 实现 `edgefleet placement show <app>`：绑定记录 + 卷注册
...
// nodesLsCmd 实现 `edgefleet nodes ls`：观测缓存只读列表（展示/诊断专用，
...
func (c *nodesLsCmd) Run(ctx context.Context, env *commands.Environment, args []string) error {
	...
	if len(views) == 0 {
		b.WriteString("  (cache empty — edgefleetd observer has not synced yet)\n")
	}
	for _, v := range views {
		id := v.PlatformID
		if id == "" {
