...
// appsCmd 是外层动词 `apps`：分发 list/get/delete/webhook/scaling。
type appsCmd struct {
	sub *commands.App
}
...
func newAppsCmd() *appsCmd {
	sub := commands.New()
	sub.Register(&appsListCmd{}, &appsGetCmd{}, &appsDeleteCmd{}, newWebhookCmd(), newScalingCmd())
	sub.VerbTitle = "apps subcommands:"
	return &appsCmd{sub: sub}
}

func (c *appsCmd) Name() string     { return "apps" }
func (c *appsCmd) Synopsis() string { return "app resources (list, details, delete, webhook, scaling)" }
func (c *appsCmd) Usage() string    { return "apps <list|get|delete|webhook|scaling> [flags] ..." }

func (c *appsCmd) SetFlags(_ *flag.FlagSet) {}

func (c *appsCmd) Run(ctx context.Context, env *commands.Environment, args []string) error {
	if len(args) == 0 {
		return &commands.UsageError{Usage: c.Usage(), Err: fmt.Errorf("missing subcommand (list|get|delete|webhook|scaling)")}
	}
