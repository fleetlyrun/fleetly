...
func newStateEnv(t *testing.T) (string, string, string) {
	...
	key := filepath.Join(dir, "fleetly.key")
...
func TestCLIPlacementShow(t *testing.T) {
	...
	if got.Constraint != "node.labels.fleetly.node-id == "+platformID {
	...
	// 人读形态含约束行。
	code, out, _ = runCLI(t, "placement", "show", "--db", db, app)
	if code != 0 || !strings.Contains(out, "constraint: node.labels.fleetly.node-id == "+platformID) {
...
func TestCLINodesLs(t *testing.T) {
	...
	if err := st.SyncNodeObservations(context.Background(), []state.SubstrateNode{{
		...
		IsManager: true, Labels: map[string]string{"fleetly.node-id": "n_test"},
	}}, time.Now()); err != nil {
		t.Fatalf("seed: %v", err)
	}
	_ = st.Close()
