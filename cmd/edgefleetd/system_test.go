...
// 验证同一 proto 契约的两面：
//   - gRPC 直连 Ping：service == "edgefleetd"、version 非空；
...
func TestPingGRPCToREST(t *testing.T) {
	...
	if gResp.GetService() != "edgefleetd" {
		t.Fatalf("gRPC service = %q, want %q", gResp.GetService(), "edgefleetd")
	}
	if gResp.GetVersion() == "" {
...
func TestValidateExemptPredicate(t *testing.T) {
	...
	if validateExempt("/edgefleet.server.v1.SystemService/Ping") {
